  ...
  me: async () => {
    ...
    if (storedToken) {
      ...
      try {
        ...
        handleError(error instanceof Error ? error.message : 'Fetch user info failed');
      }
    }
  },
...
const handleAuth = async (url: string) => {
  const { set } = secureStorage();
  try {
    const params = new URLSearchParams(new URL(url).search || new URL(url).hash.slice(1));
    ...
    handleError(error instanceof Error ? error.message : 'Authentication failed');
  }
};

export function useSession() {
  const { user, isLoading, error, signIn, me, getToken, authenticate } = useAuthStore();
